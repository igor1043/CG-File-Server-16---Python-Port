...
class Server16App(tk.Tk):
    ...
    def _show_stadium_loading_modal(self, stadium_name: str, detail: str | None = None, progress: float = 0.0) -> None:
    ...
    def _build_stadium_card(self, parent: tk.Misc, row: int) -> None:
        ...
        self.progress_text = tk.Label(card, text=self.display_value("idle"), bg=self.card, fg=self.muted, font=("Bahnschrift", 9))
    ...
    def _load_team_database(self, progress_callback=None) -> None:
        ...
        try:
            ...
            if progress_callback is not None:
                progress_callback(50, self.progress_text("connecting_database"))
            if self.team_db.connect():
                if progress_callback is not None:
                    progress_callback(72, self.progress_text("loading_teams"))
                ...
                if progress_callback is not None:
                    progress_callback(78, self.progress_text("database_ready"))
            else:
                reason = self.team_db.last_error if self.team_db else "unknown reason"
                self.log(f"️  Could not connect to team database: {reason}")
                self.team_db = None
                if progress_callback is not None:
                    progress_callback(78, self.progress_text("database_unavailable"))
        except Exception as e:
            self.log(f"❌ Error loading team database: {e}")
            self.team_db = None
            if progress_callback is not None:
                progress_callback(78, self.progress_text("database_failed"))
